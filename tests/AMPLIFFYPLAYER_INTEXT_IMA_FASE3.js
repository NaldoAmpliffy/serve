...
var Ava = /*#__PURE__*/function () {
  function Ava(domElement, rootElement, playerStorage, position, viewPortSize, avaSize, resizeFn, delayStartStop) {
    _classCallCheck(this, Ava);
    ...
    this.wantsOn = false;
    this.observer = [];
    this.delayStartStop = delayStartStop;
  }
  _createClass(Ava, [{
    key: "startAva",
    value: function startAva() {
      ...
      if (this.errorWithDomElement()) return false;
      this.wantsOn = true;
      if (!this.delayStartStop) return this.actuallyStartAva();
      this.manageDelayStartStop();
      return false;
    }
  }, {
    key: "actuallyStartAva",
    value: function actuallyStartAva() {
      this.size = [this.domElement.parentElement.clientWidth, this.domElement.parentElement.clientHeight];
      this.doSnapshot();
      this.domElement.parentElement.style.width = this.size[0] + 'px';
      this.domElement.parentElement.style.height = this.size[1] + 'px';
      this.doFix();
      this.on = true;
      this.playerStorage.playerSize.setSizeByType('ava', this.avaSize.getWidth(), this.avaSize.getHeight());
      this.playerStorage.playerSize.setType('ava');
      if (this.resizeFn) this.resizeFn({
        width: this.avaSize.getWidth(),
        height: this.avaSize.getHeight()
      });
      this.observer.forEach(function (x) {
        return x();
      });
      return true;
    }
    ...
    key: "stopAva",
    value: function stopAva() {
      if (!this.on) return false;
      if (this.forceStop) return false;
      if (this.errorWithDomElement()) return false;
      this.wantsOn = false;
      if (!this.delayStartStop) return this.actuallyStopAva();
      this.manageDelayStartStop();
      return false;
    }
  }, {
    key: "actuallyStopAva",
    value: function actuallyStopAva() {
      this.doUnfix();
      this.doRestore();
      this.on = false;
      this.playerStorage.playerSize.setType('nonAva');
      this.observer.forEach(function (x) {
        return x();
      });
      return true;
    ...
    key: "doSnapshot",
    value: function doSnapshot() {
      if (this.errorWithDomElement()) return false;
      this.snapshot.parentElement.width = this.domElement.parentElement.style.width;
      this.snapshot.parentElement.height = this.domElement.parentElement.style.height;
      if (this.resizeFn) this.snapshot.player = this.resizeFn();
      this.snapshot.width = this.domElement.style.width;
      this.snapshot.height = this.domElement.style.height;
      this.snapshot.zIndex = this.domElement.style.zIndex;
      this.snapshot.position = this.domElement.style.position;
      switch (this.position) {
        case 'top-left':
          this.snapshot.top = this.domElement.style.top;
          this.snapshot.left = this.domElement.style.left;
          break;
        case 'top-right':
          this.snapshot.top = this.domElement.style.top;
          this.snapshot.right = this.domElement.style.right;
          break;
        case 'bottom-left':
          this.snapshot.bottom = this.domElement.style.bottom;
          this.snapshot.left = this.domElement.style.left;
          break;
        case 'bottom-right':
          this.snapshot.bottom = this.domElement.style.bottom;
    ...
    value: function doRestore() {
      if (this.errorWithDomElement()) return false;
      if (typeof this.snapshot.parentElement.width !== 'undefined') this.domElement.parentElement.style.width = this.snapshot.parentElement.width;
      if (typeof this.snapshot.parentElement.height !== 'undefined') this.domElement.parentElement.style.height = this.snapshot.parentElement.height;
      if (this.resizeFn) this.snapshot.player = this.resizeFn(this.snapshot.player);
    }
  }, {
    key: "doFix",
    ...
    key: "doUnfix",
    value: function doUnfix() {
      return true;
    }
  }, {
    key: "errorWithDomElement",
    value: function errorWithDomElement() {
      var isHTMLElement = this.domElement.parentElement instanceof this.domElement.parentElement.ownerDocument.defaultView.HTMLElement || this.domElement.parentElement.toString().match(/\[object HTML.*Element\]/);
      return !this.domElement || !this.domElement.parentElement || !isHTMLElement;
    }
  }, {
    key: "manageDelayStartStop",
    value: function manageDelayStartStop() {
      if (!this.delayStartStop) return;
      this.delayStartStop(this.handleDelayStartStop.bind(this), 50);
    }
  }, {
    key: "handleDelayStartStop",
    value: function handleDelayStartStop() {
      if (this.wantsOn && !this.on) {
        this.actuallyStartAva();
      }
      if (!this.wantsOn && this.on) {
        this.actuallyStopAva();
      }
    }
  }]);
  return Ava;
...
var AvaCSS = /*#__PURE__*/function (_Ava) {
  _inherits(AvaCSS, _Ava);
  var _super = _createSuper(AvaCSS);
  function AvaCSS() {
    _classCallCheck(this, AvaCSS);
  ...
  _createClass(AvaCSS, [{
    ...
    value: function setupCSS() {
      ...
      if (!parent) return;
      var name = this.setupCSSClassName();
      var cssElement = this.cssElement = parent.ownerDocument.createElement('style');
      cssElement.type = 'text/css';
      var transitionProperties = 'top,left,right,bottom,position';
      var transitionDuration = '0.4s,0.4s,0.4s,0.4s,0.4s'; // Must be less than 0.5 to avoid CLS issues
      cssElement.innerHTML = '.' + name + 'fixed { position: fixed; z-index: 20000000; transition: ' + transitionProperties + ' ' + transitionDuration + ' ease-in-out; width: ' + this.avaSize.getWidth() + 'px; height: ' + this.avaSize.getHeight() + 'px; }' + '.' + name + 'top-left { top: 10px; left: 10px; right: unset; bottom: unset; }' + '.' + name + 'top-right { top: 10px; right: 10px; left: unset; bottom: unset; }' + '.' + name + 'bottom-left { bottom: 10px; left: 10px; right: unset; top: unset; }' + '.' + name + 'bottom-right { bottom: 10px; right: 10px; left: unset; top: unset; }';
      var root = this.domElement;
      while (root && root.parentElement && root.parentElement !== root) root = root.parentElement;
      root.querySelector('head').appendChild(cssElement);
    }
  }, {
    key: "setupCSSClassName",
    value: function setupCSSClassName() {
      if (!this.cssClassName) this.cssClassName = 'avaCSS' + new Date().getTime();
      return this.cssClassName;
    }
  }, {
    key: "setupUnfixedCSS",
    value: function setupUnfixedCSS() {
      var parent = this.domElement.parentElement;
      if (!parent) return;
      if (this.cssUnfixedElement) {
        this.cssUnfixedElement.remove();
        this.cssUnfixedElement = undefined;
      }
      var name = this.setupCSSClassName();
      var bounding = parent.getBoundingClientRect();
      (0, _log.cLog)('bounding:', bounding);
      var cssElement = this.cssUnfixedElement = parent.ownerDocument.createElement('style');
      cssElement.type = 'text/css';
      var transitionProperties = 'top,left,right,bottom,position';
      var transitionDuration = '0.4s,0.4s,0.4s,0.4s,0.4s'; // Must be less than 0.5 to avoid CLS issues
      ...
      switch (this.position) {
        case 'top-left':
          coords = 'top: ' + bounding.top + 'px; left:' + bounding.left + 'px; right: initial; bottom: initial;';
          break;
        case 'top-right':
          coords = 'top: ' + bounding.top + 'px; left: initial; right:' + (viewPortSize[0] - bounding.right) + 'px; bottom: initial;';
          break;
        case 'bottom-left':
          coords = 'top: initial; left:' + bounding.left + 'px; right: initial; bottom: ' + (viewPortSize[1] - bounding.bottom) + 'px;';
          break;
        case 'bottom-right':
          coords = 'top: initial; left: initial; right: ' + (viewPortSize[0] - bounding.right) + 'px; bottom: ' + (viewPortSize[1] - bounding.bottom) + 'px;';
          break;
      }
      cssElement.innerHTML = '.' + name + 'unfixed { position:fixed; z-index: 20000000; ' + coords + ' transition: ' + transitionProperties + ' ' + transitionDuration + ' ease-in-out; }';
      this.rootElement.querySelector('head').appendChild(cssElement);
    }
  }, {
    key: "doFix",
    value: function doFix() {
      this.setupCSS();
      this.setupUnfixedCSS();
      var name = this.cssClassName;
      if (!name) throw "Missing cssClassName";
      var iframe = this.playerStorage.getElementToFloat();
      this.domElement.classList.add(name + 'unfixed');
      if (iframe) iframe.classList.add(name + 'unfixed');
      // Chrome v111 has changed the way it handles Stacking Contexts, and we need to make sure we float above other elements
      if (this.playerStorage.rootDocument && this.playerStorage.rootDocument.nodeName && this.playerStorage.rootDocument.nodeName.match && this.playerStorage.rootDocument.nodeName.match(/-fragment/)) {
        if (this.playerStorage.rootDocument.host) {
          this.playerStorage.rootDocument.host.style.zIndex = 1111;
          var styleMap = window.getComputedStyle(this.playerStorage.rootDocument.host);
          if (styleMap.position !== 'absolute' || styleMap.position !== 'relative') {
            if (this.playerStorage.rootDocument.host.style.top.match(/px$/) || this.playerStorage.rootDocument.host.style.left.match(/px$/)) {
              this.playerStorage.rootDocument.host.style.position = 'absolute';
            } else this.playerStorage.rootDocument.host.style.position = 'relative';
          }
        }
      }
      var myThis = this;
      requestAnimationFrame(function (name) {
        if (myThis.wantsOn) {
          // iOS may get lagged and reach here too late
          myThis.domElement.classList.remove(name + 'unfixed');
          if (iframe) iframe.classList.remove(name + 'unfixed');
          myThis.domElement.classList.add(name + 'fixed');
          if (iframe) iframe.classList.add(name + 'fixed');
          myThis.domElement.classList.add(name + myThis.position);
          if (iframe) iframe.classList.add(name + myThis.position);
        }
      }.bind(this, name));
      return true;
    }
    ...
    value: function doUnfix() {
      ...
      this.domElement.classList.add(name + 'unfixed');
      if (iframe) iframe.classList.add(name + 'unfixed');
      this.domElement.classList.remove(name + 'fixed');
      if (iframe) iframe.classList.remove(name + 'fixed');
      this.domElement.classList.remove(name + this.position);
      if (iframe) iframe.classList.remove(name + this.position);
      // Chrome v111 has changed the way it handles Stacking Contexts, and we need to make sure we float above other elements
      if (this.timeoutID) clearTimeout(this.timeoutID);
      var myThis = this;
      this.timeoutID = setTimeout(function (name) {
        if (!myThis.wantsOn) {
          // iOS may get lagged and reach here too late
          myThis.domElement.classList.remove(name + 'unfixed');
          if (iframe) iframe.classList.remove(name + 'unfixed');
          if (myThis.resizeFn) myThis.resizeFn(myThis.snapshot.player);
          if (myThis.resizeFn) setTimeout(myThis.resizeFn.bind(myThis, myThis.snapshot.player), 150);
        }
      }.bind(this, name), 950);
      return true;
    }
...
function _isNativeReflectConstruct() { if (typeof Reflect === "undefined" || !Reflect.construct) return false; if (Reflect.construct.sham) return false; if (typeof Proxy === "function") return true; try { Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], function () {})); return true; } catch (e) { return false; } }
function _getPrototypeOf(o) { _getPrototypeOf = Object.setPrototypeOf ? Object.getPrototypeOf.bind() : function _getPrototypeOf(o) { return o.__proto__ || Object.getPrototypeOf(o); }; return _getPrototypeOf(o); }
var AvaSizeDynamic = /*#__PURE__*/function (_IsMobile) {
  _inherits(AvaSizeDynamic, _IsMobile);
  var _super = _createSuper(AvaSizeDynamic);
  function AvaSizeDynamic(viewPortSize) {
    _classCallCheck(this, AvaSizeDynamic);
    return _super.call.apply(_super, [this].concat(_toConsumableArray(viewPortSize)));
  }
  ...
  width * height = percentage * viewPortWidth * viewPortHeight
  height = width / ratio
  =>
  width * width / ratio = percentage * viewPortWidth * viewPortHeight
  =>
  width = sqrt(percentage * viewPortWidth * viewPortHeight * ratio)
   */
  _createClass(AvaSizeDynamic, [{
    key: "getWidth",
    value: function getWidth() {
      var viewPortArea = this.viewPortWidth * this.viewPortHeight;
      var mobileWidth = Math.sqrt(viewPortArea * (this._avaSize.mobile.percentage / 100) * this._ratio);
      return this.isMobile() ? Math.min(this.viewPortWidth, mobileWidth) : this._avaSize.desktop.width;
    }
  }, {
    key: "getHeight",
    value: function getHeight() {
      return this.isMobile() ? this.getWidth() / this._ratio : this._avaSize.desktop.height;
    }
  }, {
    key: "avaSize",
  ...
  return AvaSizeDynamic;
}(_isMobile.IsMobile);
exports.AvaSizeDynamic = AvaSizeDynamic;
...
function _isNativeReflectConstruct() { if (typeof Reflect === "undefined" || !Reflect.construct) return false; if (Reflect.construct.sham) return false; if (typeof Proxy === "function") return true; try { Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], function () {})); return true; } catch (e) { return false; } }
function _getPrototypeOf(o) { _getPrototypeOf = Object.setPrototypeOf ? Object.getPrototypeOf.bind() : function _getPrototypeOf(o) { return o.__proto__ || Object.getPrototypeOf(o); }; return _getPrototypeOf(o); }
var AvaStyles = /*#__PURE__*/function (_Ava) {
  _inherits(AvaStyles, _Ava);
  var _super = _createSuper(AvaStyles);
  function AvaStyles() {
    _classCallCheck(this, AvaStyles);
    return _super.apply(this, arguments);
  }
  _createClass(AvaStyles, [{
    key: "doFix",
    value: function doFix() {
      var iframe = this.domElement;
      this.domElement.style.width = this.avaSize.getWidth() + 'px';
      this.domElement.style.height = this.avaSize.getHeight() + 'px';
      if (iframe) {
        iframe.style.width = this.avaSize.getWidth() + 'px';
        iframe.style.height = this.avaSize.getHeight() + 'px';
      }
      switch (this.position) {
        case 'top-left':
      ...
      this.domElement.style.zIndex = '' + 2E6;
    ...
    value: function doUnfix() {
      if (this.errorWithDomElement()) return false;
      var iframe = this.domElement;
      if (typeof this.snapshot.width !== 'undefined') this.domElement.style.width = this.snapshot.width;
      if (typeof this.snapshot.height !== 'undefined') this.domElement.style.height = this.snapshot.height;
      if (typeof this.snapshot.zIndex !== 'undefined') this.domElement.style.zIndex = this.snapshot.zIndex;
      if (typeof this.snapshot.position !== 'undefined') this.domElement.style.position = this.snapshot.position;
      switch (this.position) {
        case 'top-left':
          if (typeof this.snapshot.top !== 'undefined') this.domElement.style.top = this.snapshot.top;
          if (typeof this.snapshot.left !== 'undefined') this.domElement.style.left = this.snapshot.left;
          break;
        case 'top-right':
          if (typeof this.snapshot.top !== 'undefined') this.domElement.style.top = this.snapshot.top;
          if (typeof this.snapshot.right !== 'undefined') this.domElement.style.right = this.snapshot.right;
          break;
        case 'bottom-left':
          if (typeof this.snapshot.bottom !== 'undefined') this.domElement.style.bottom = this.snapshot.bottom;
          if (typeof this.snapshot.left !== 'undefined') this.domElement.style.left = this.snapshot.left;
          break;
        case 'bottom-right':
          if (typeof this.snapshot.bottom !== 'undefined') this.domElement.style.bottom = this.snapshot.bottom;
      ...
      if (iframe) {
        iframe.style.width = this.snapshot.width ? this.snapshot.width : '';
        iframe.style.height = this.snapshot.height ? this.snapshot.height : '';
      }
      return true;
    }
  }]);
...
var Controller = /*#__PURE__*/function () {
  ...
  _createClass(Controller, [{
    ...
    value: function changeAvaStartStop() {
      if (this.wantsAva) this.ava.startAva();else this.ava.stopAva();
    }

    /*
     @return boolean true if we have started the AVA
     */
  }, {
    key: "startAva",
    value: function startAva() {
    ...
    key: "forceStartAva",
    value: function forceStartAva() {
      this.viewedCompletelyTS = performance.now();
      this.wantsAva = true;
      this.ava.startAva();
    }
  }, {
    key: "viewableCompletely",
    value: function viewableCompletely() {
      var force = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
      var mustCheckAVA = force || this.viewedCompletelyTS;
      if (!this.viewedCompletelyTS) this.viewedCompletelyTS = performance.now();
      if (mustCheckAVA) {
        if (this.wantsAva) {
          this.wantsAva = false;
          this.debouncedChangeAvaStartStop();
          this.stopTimer();
        }
      }
    }
    ...
    value: function startTimer() {
      ...
      this.timer = setInterval(this.checkInterval.bind(this), 1000);
    }
  }, {
    key: "stopTimer",
    ...
    value: function checkInterval() {
      if (!this.wantsAva) return;
      if (this.startAva()) this.stopTimer();
    }
  }]);
  return Controller;
}();
...
var _viewability = require("./viewability");
var _avaCSS = require("./avaCSS");
var _viewport = require("../dom/viewport");
var _avaStyles = require("./avaStyles");
var _closeCSS = require("../interstitial/closeCSS");
var _avaSizeDynamic = require("./avaSizeDynamic");
var _log = require("../log/log");
var _cls = require("../browser/cls");
function install(videoElement, domElementToAVA, rootElementToAVA, domElementToObserve, position, player, playerStorage, avaSize) {
  var closeSize = arguments.length > 8 && arguments[8] !== undefined ? arguments[8] : undefined;
  var avaCloseDelayOnVidAd = arguments.length > 9 && arguments[9] !== undefined ? arguments[9] : -1;
  var avaOnlyOnVidAd = arguments.length > 10 && arguments[10] !== undefined ? arguments[10] : false;
  var startAVAOnSetup = arguments.length > 11 && arguments[11] !== undefined ? arguments[11] : false;
  var startAVAOnVidAdStart = arguments.length > 12 && arguments[12] !== undefined ? arguments[12] : false;
  var observers = arguments.length > 13 && arguments[13] !== undefined ? arguments[13] : null;
  var ava;
  var getViewPortSize = _viewport.getTopViewPortSize;
  var viewPortSize = getViewPortSize();
  var resizeFn = function resizeFn(size) {
    if (size) {
      playerStorage.player.width(size.width);
      playerStorage.player.height(size.height);
      try {
        var adsManager = playerStorage.player.ima.getAdsManager();
        adsManager.resize(size.width, size.height, 'NORMAL');
      } catch (e) {
        (0, _log.cLog)("AdsManager not set", e);
      }
    }
    return {
      width: playerStorage.player.width(),
      height: playerStorage.player.height()
    };
  };
  var avaSizeClass = new _avaSizeDynamic.AvaSizeDynamic(viewPortSize);
  avaSizeClass.avaSize = playerStorage.options.avaSize;
  var width = playerStorage.player.width();
  var height = playerStorage.player.height();
  avaSizeClass.ratio = width && height ? width / height : 16 / 9;
  var addClsObserver = undefined;
  if (playerStorage.options.avaAvoidCLS) {
    var cls = new _cls.Cls();
    cls.setup();
    addClsObserver = playerStorage.options.avaAvoidCLS ? cls.addObserver.bind(cls) : undefined;
  }
  var isSafari = 0 && /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
  if (isSafari) ava = new _avaStyles.AvaStyles(domElementToAVA, rootElementToAVA, playerStorage, position, getViewPortSize, avaSizeClass, resizeFn, addClsObserver);else ava = new _avaCSS.AvaCSS(domElementToAVA, rootElementToAVA, playerStorage, position, getViewPortSize, avaSizeClass, resizeFn, addClsObserver);
  playerStorage.pollForceViewability(function () {
    return ava.on;
  });
  playerStorage.pollIsAvaOn = function () {
    return ava.on;
  };
  if (typeof playerStorage.setForceViewabilityCheck === 'function') ava.observer.push(playerStorage.setForceViewabilityCheck);
  var closeAVA = new _closeCSS.CloseCSS(videoElement.parentElement, rootElementToAVA, function () {
    playerStorage.playerState.adManager.userClosedVidAd = true;
    playerStorage.player.one('playing', function () {
      return playerStorage.playerState.adManager.userClosedVidAd = false;
    });
    ava.stopAva();
    setTimeout(function () {
      (0, _log.cLog)("AVA close => stop playing", playerStorage.playerState.adManager.playingAnAd);
      if (playerStorage.playerState.adManager.playingAnAd) {
        // We only pause the Ad if sound is on.
        // If there is no sound, then we allow the ad to reach Completion
        if (!playerStorage.player.muted() || playerStorage.player.volume() === 0) playerStorage.player.ima.pauseAd();
      } else if (!playerStorage.player.paused()) {
        (0, _log.cLog)("AVA close => pause playing");
        playerStorage.player.pause();
      }
    }, 100);
    /* ava.forceStop = true; */
    if (observers && typeof observers.avaClose !== 'undefined' && Array.isArray(observers.avaClose)) observers.avaClose.forEach(function (x) {
      return x();
    });
  });
  closeAVA.size = closeSize;
  closeAVA.setup();
  closeAVA.hide();
  if (avaCloseDelayOnVidAd >= 0) {
    if (avaCloseDelayOnVidAd) {
      playerStorage.player.on(['adstart', 'adsstart'], function () {
        (0, _log.cLog)("hideDuringSomeMilliseconds", avaCloseDelayOnVidAd);
        closeAVA.hideDuringSomeMilliseconds(avaCloseDelayOnVidAd * 1000);
      });
    }
  }
  ava.observer.push(function (ava, closeAVA) {
    if (ava.on) closeAVA.show();else closeAVA.hide();
  }.bind(this, ava, closeAVA));
  (0, _viewability.avaViewabilityLogic)(ava, domElementToObserve, playerStorage.player, playerStorage, undefined, undefined, 2000, startAVAOnSetup, avaOnlyOnVidAd, startAVAOnVidAdStart);
  return ava;
}
...
exports.avaViewabilityLogic = avaViewabilityLogic;
...
function avaViewabilityLogic(ava, containerDomElement, player, playerStorage, viewabilityThresholdPlayer100, viewabilityThresholdPlayer80) {
  ...
  var avaOnlyOnVidAd = arguments.length > 8 && arguments[8] !== undefined ? arguments[8] : false;
  var startAVAOnVidAdStart = arguments.length > 9 && arguments[9] !== undefined ? arguments[9] : false;
  var statusViewablePlayer100 = {
    threshold: 0.99,
    viewable: undefined,
  ...
  var statuses = [statusViewablePlayer100, statusViewablePlayer80];
  var observer;
  ...
  if (avaOnlyOnVidAd) {
    // We only show AVA while on VidAd.
    var playingVidAdState = null;
    var showAvaOnVidAd = function showAvaOnVidAd() {
      //console.log("VAST AVA: playerState.playingVidAd:",playerState.playingVidAd,"playingVidAdState",playingVidAdState)
      if (playerStorage.playerState.playingVidAd && !playerStorage.playerState.adManager.userClosedVidAd) {
        playingVidAdState = true;
        //console.log("VAST AVA: Force Start");
        return avaController.startAva();
      } else if (playingVidAdState || avaController.ava.on) {
        // We were playing a VidAd, but no more
        // It was playing VidAd but no more => We disable AVA
        //console.log("VAST AVA: Force Stop");
        avaController.wantsAva = false;
        avaController.changeAvaStartStop();
        playingVidAdState = false;
      }
    };
    // If less viewable than 80% => we start AVA when vidAd is playing, we stop it otherwise
    statusViewablePlayer80.observer.push(function (state) {
      if (state.viewable) {
        avaController.wantsAva = false;
        avaController.changeAvaStartStop();
        playingVidAdState = false;
        if (timerKeepTrackVidAd) {
          clearInterval(timerKeepTrackVidAd);
          timerKeepTrackVidAd = null;
        }
        return;
      }
      showAvaOnVidAd();
      if (!timerKeepTrackVidAd) timerKeepTrackVidAd = setInterval(showAvaOnVidAd, 1000);
    });
    statusViewablePlayer100.observer.push(function (state) {
      if (state.viewable) {
        /*
            if(timerKeepTrackVidAd) {
                clearInterval(timerKeepTrackVidAd);
         ...
         */
        return avaController.viewableCompletely(true);
      }
      return null;
    });
  } else {
    statusViewablePlayer80.observer.push(function (state) {
      return state.viewable ? null : avaController.startAva();
    });
    statusViewablePlayer100.observer.push(function (state) {
      return state.viewable ? avaController.viewableCompletely() : null;
    });
  }
  observer = new _intersection.Intersection(statuses);
  observer.setup(containerDomElement);
...
var Cls = /*#__PURE__*/function () {
  function Cls() {
    ...
    this.observers = [];
    this.timeSinceUserInteractedWithThePage = new Date().getTime();
  }
  _createClass(Cls, [{
    key: "setup",
    ...
    key: "addCLSEvents",
    value: function addCLSEvents() {
      top.addEventListener('touchend', this.updateTimeSinceUserInteractedWithThePage.bind(this));
      top.addEventListener('keydown', this.updateTimeSinceUserInteractedWithThePage.bind(this));
      top.addEventListener('mousedown', this.updateTimeSinceUserInteractedWithThePage.bind(this));
    }
  }, {
    key: "updateTimeSinceUserInteractedWithThePage",
    value: function updateTimeSinceUserInteractedWithThePage() {
      this.timeSinceUserInteractedWithThePage = new Date().getTime();
      this.dispatchEvents();
    }
  }, {
    ...
    value: function addObserver(observer) {
      var marginInMS = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : undefined;
      if (marginInMS) {
        var now = new Date().getTime();
        if (now - this.timeSinceUserInteractedWithThePage < marginInMS) {
          observer();
          return;
        }
      }
...
var Close = /*#__PURE__*/function () {
  function Close(domElement, rootElement, onClose) {
    var disposeOnClose = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
...
function _isNativeReflectConstruct() { if (typeof Reflect === "undefined" || !Reflect.construct) return false; if (Reflect.construct.sham) return false; if (typeof Proxy === "function") return true; try { Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], function () {})); return true; } catch (e) { return false; } }
function _getPrototypeOf(o) { _getPrototypeOf = Object.setPrototypeOf ? Object.getPrototypeOf.bind() : function _getPrototypeOf(o) { return o.__proto__ || Object.getPrototypeOf(o); }; return _getPrototypeOf(o); }
var CloseCSS = /*#__PURE__*/function (_Close) {
  _inherits(CloseCSS, _Close);
  var _super = _createSuper(CloseCSS);
  function CloseCSS() {
    _classCallCheck(this, CloseCSS);
    return _super.apply(this, arguments);
  }
  _createClass(CloseCSS, [{
    key: "setup",
    value: function setup() {
      this.setupCSS();
      this.wantsToBeShown = true;
      this.forceHidden = false;
    }
  }, {
    key: "show",
    value: function show() {
      if (!this.domCloseElement) throw "show without an element";
      this.wantsToBeShown = true;
      this.applyShowHide();
    }
  }, {
    key: "hide",
    value: function hide() {
      if (!this.domCloseElement) throw "hide without an element";
      this.wantsToBeShown = false;
      this.applyShowHide();
    }
  }, {
    key: "applyShowHide",
    value: function applyShowHide() {
      if (this.wantsToBeShown && !this.forceHidden) this.domCloseElement.style.display = '';else this.domCloseElement.style.display = 'none';
    }
  }, {
    key: "hideDuringSomeMilliseconds",
    value: function hideDuringSomeMilliseconds(milliseconds) {
      var _this = this;
    ...
    value: function setupCSS() {
      if (this.cssElement) return;
      var size = this.size ? this.size : 20;
      var domElement = this.domElement;
      if (!domElement) return;
      var name = this.setupCSSClassName();
      var closeElement = this.domCloseElement = domElement.ownerDocument.createElement('a');
      closeElement.className = name;
      closeElement.onclick = this.closeClicked.bind(this);
      this.domElement.appendChild(closeElement);
      //this.domElement.style.position = 'relative';
      var cssElement = this.cssElement = domElement.ownerDocument.createElement('style');
      cssElement.type = 'text/css';
      cssElement.innerHTML = '.' + name + ' {\n' + '  color: white;\n' + '  font: ' + size * .9 + 'px/100% arial, sans-serif;\n' + '  position: absolute;\n' + '  right: 5px;\n' + '  text-decoration: none;\n' + '  text-shadow: 0 1px 0 #fff;\n' + '  top: 5px;\n' + '  z-index: 200000000;\n' + '    width: ' + size + 'px;\n' + '    height: ' + size + 'px;\n' + '    background: black;\n' + '    text-align: center;\n' + '    line-height: ' + size + 'px;\n' + '    cursor: pointer;' + '}' + '.' + name + ':after { content: \'X\'; z-index: 200000000; /* UTF-8 symbol */ }';
      this.rootElement.querySelector('head').appendChild(cssElement);
    }
  }, {
    key: "setupCSSClassName",
    value: function setupCSSClassName() {
      if (!this.cssClassName) this.cssClassName = 'closeCSS' + new Date().getTime();
      return this.cssClassName;
    }
  }, {
    key: "dispose",
    value: function dispose() {
      if (this.domCloseElement) {
        this.domCloseElement.remove();
        this.domCloseElement = undefined;
      }
      if (this.cssElement) {
...
var _setup = require("./ava/setup");
var _closeCSS = require("./interstitial/closeCSS");
var _trackAdRequestDone = require("./player/trackAdRequestDone");
var _consent = require("./consent/consent");
var _playlistGAM = require("./vidCo/playlistGAM");
var _log = require("./log/log");
var _isMobile = require("./dom/isMobile");
var _viewport = require("./dom/viewport");
var _bringGoogleIMAObjectsToThisWindow = require("./player/bringGoogleIMAObjectsToThisWindow");
function _typeof(obj) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (obj) { return typeof obj; } : function (obj) { return obj && "function" == typeof Symbol && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }, _typeof(obj); }
...
function _arrayWithoutHoles(arr) { if (Array.isArray(arr)) return _arrayLikeToArray(arr); }
function _arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) arr2[i] = arr[i]; return arr2; }
var playerStorage = top.ampTV;

//console.log("Loaded! playerStorage", playerStorage);
var videojs = playerStorage.videojs;
var oldRequire = global.require;
global.require = function (module) {
  if (module === 'video.js') return videojs;
  if (oldRequire) return oldRequire(module);
};
function addIMA3Script(rootDocument, useShadowRoot, ima_loaded, ima3RequiresAnAccessibleScript) {
  var dev = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
  var ima_script = document.createElement('script');
  ima_script.type = "text/javascript";
  if (dev) ima_script.src = "https://imasdk.googleapis.com/js/sdkloader/ima3.js";else ima_script.src = "https://imasdk.googleapis.com/js/sdkloader/ima3_debug.js";
  ima_script.onload = ima_loaded;
  ima3RequiresAnAccessibleScript(ima_script.src, useShadowRoot, top.document);
  var placeholder = rootDocument.querySelector('head') || rootDocument.head || rootDocument;
  placeholder.appendChild(ima_script);
}
var ima3RequiresAnAccessibleScript = function ima3RequiresAnAccessibleScript(imaURL, useShadowRoot, doc) {
  if (!useShadowRoot) return;
  var className = 'ima-needs-this-to-continue';
  if (doc.querySelector("." + className)) return;
  var ima3 = doc.createElement('div');
  ima3.style.display = 'none';
  doc.body.appendChild(ima3);
  ima3.innerHTML = '<script src="' + imaURL + '" class="' + className + '"></script>';
};
var ampPhase3 = function ampPhase3(configID) {
  if (typeof top.ampTV.ampPhase3 === 'undefined') top.ampTV.ampPhase3 = ampPhase3;
  var playerConfig = top.ampTV.getConfigByElement(configID);
  var playerStorage = playerConfig.data;
  //console.log("Loaded! PHASE 3",configID,playerStorage);

  var player = playerStorage.player;
  var videoEl = playerStorage.rootDocument.querySelector('#' + player.id());
  exports['video.js'] = videojs;
  require("videojs-ima");
  var vidCoObj = playerStorage.vidCoObj;
  var onResolved = function onResolved(player) {
    this.switchPlaylist(player);
    this.hookPlayer(player);
  };
  var resolved = vidCoObj.resolveMissing(playerStorage.options.size, onResolved.bind(vidCoObj, player));
  if (!resolved && vidCoObj.playList.length) onResolved.bind(vidCoObj)(player);
  (0, _playlistGAM.monitorPlaylist)(vidCoObj, playerStorage.options, player, playerStorage.options.playListPreloadNum);
  playerStorage.playerState.adManager.getAdTag = playerStorage.vidAdQueue.getAdTag.bind(playerStorage.vidAdQueue);
  var ima_loaded = function ima_loaded() {
    /*
    ...
    */
    /*    function resetVideoElement(id,ready) {
            const videojsContainer = document.getElementById(id);
            let videoEl = videojsContainer.querySelector('video');
            window.snapshot = {
                ...
                muted: videoEl.muted,
                controls: player.controls() || videoEl.getAttribute('controls'),
                id: id,
                className: videojsContainer.className,
                parent: videojsContainer.parentNode,
                next: videojsContainer.nextSibling,
                playList: player.playlist(),
                playListCurrentItem: player.playlist.currentIndex(),
                width: player.width(),
            ...
            videoEl.id = snapshot.id;
            if(window.snapshot.controls) videoEl.setAttribute('controls','controls');
            videoEl.className = snapshot.className;
            window.snapshot.parent.insertBefore(videoEl, window.snapshot.next);
            const options = {
                autoplay: !window.snapshot.paused,
                muted: window.snapshot.muted,
                controls: window.snapshot.controls,
                src: window.snapshot.src,
                poster:window.snapshot.poster,
                width:window.snapshot.width,
                height:window.snapshot.height
            };
            global.player = player = videojs(id,options,function() {
                (ready.bind(this))();
                this.playlist(window.snapshot.playList);
                this.playlist.currentIndex(window.snapshot.playListCurrentItem);
                this.currentTime(window.snapshot.currentTime);
                this.bcPlaylistUi({autoadvance:0,playOnSelect:true,hideOnStart:!window.snapshot.paused});
                player.on('timeupdate', window.onTimeUpdate);
            });
        }*/

    if (playerStorage.options.dev) {
      playerStorage.videojs.log.level('debug');
    }
    var adsManagerIMA;
    var onAdsManagerLoaded = function onAdsManagerLoaded() {
      (0, _log.cLog)('AMP: onAdsManagerLoaded');
      // See https://github.com/googleads/videojs-ima/issues/406 -> allow to receive the LOADED event as soon as posible
      player.ima.addEventListener(google.ima.AdEvent.Type.LOADED, function (event) {
        (0, _log.cLog)('AMP: LOADED event');
        adsManagerIMA = player.ima.getAdsManager();
        ...
        if (playerStorage.options.vidAdMaxTime) {
          setTimeout(function () {
            ...
            (0, _log.cLog)('[amp] Trigger end Ad manually. Controlled by vidAdMaxTime option.');
      ...
      if (mobile.isMobile()) {} else {
        adsManagerIMA = player.ima.getAdsManager();
        //console.log("CLICK event registered", google.ima.AdEvent.Type.CLICK, "adsManager:", adsManagerIMA);
        adsManagerIMA.addEventListener(google.ima.AdEvent.Type.CLICK, function (event) {
          //console.log('CLICK event');
          player.muted(true);
          adsManagerIMA.setVolume(0);
          setTimeout(player.ima.resumeAd.bind(window), 50);
        });
      }
      var elementToFloat = playerStorage.getElementToFloat();
      var adsManagerWidth, adsManagerHeight;
      setInterval(function () {
        if (!document.fullscreenElement && elementToFloat) {
          var change = false;
          var frameElementWidth = Math.round(elementToFloat.clientWidth);
          var frameElementHeight = Math.round(elementToFloat.clientHeight);
          if (!frameElementWidth || !frameElementHeight) return;
          var playerWidth = Math.round(player.width());
          var playerHeight = Math.round(player.height());
          if (playerWidth && frameElementWidth !== playerWidth) {
            player.width(elementToFloat.clientWidth);
            change = true;
          }
          if (playerHeight && frameElementHeight !== playerHeight) {
            player.height(elementToFloat.clientHeight);
            change = true;
          }
          var ima3Change = !adsManagerWidth || !adsManagerHeight || adsManagerWidth !== frameElementWidth || adsManagerHeight !== frameElementHeight;
          if ((change || ima3Change) && adsManagerIMA) {
            adsManagerWidth = frameElementWidth;
            adsManagerHeight = frameElementHeight;
            adsManagerIMA.resize(frameElementWidth, frameElementHeight, 'NORMAL');
            //console.log("Loaded! Resize IMA",elementToFloat,frameElementWidth, frameElementHeight, 'NORMAL')
          }
        }
      }, 1000);
    };
    var ready = function ready() {
      ...
      try {
        var loc = top.document.location.hostname;
        //console.log('vpaidModeForced location', loc);
        var mobile = _construct(_isMobile.IsMobile, _toConsumableArray((0, _viewport.getTopViewPortSize)()));
        if (!mobile.isMobile() && loc.match(/(\.ahorradororata\.com|\.ascodevida\.com|\.asivaespana\.com|\.coronaviral\.es|\.cuantafauna\.com|\.cuantarazon\.com|\.cuantocabron\.com|\.humorenserie\.com|\.memedeportes\.com|\.notengotele\.com|\.teniaquedecirlo\.com|\.vayagif\.com|\.viralizalo\.com|\.vrutal\.com|\.vistoenlasredes\.com|urbanian\.mundodeportivo\.com)$/)) {
          vpaidModeForced = 'SECURE';
        }
      } catch (e) {}
      //console.log('vpaidModeForced', vpaidModeForced)
      var imaOptions = {
        id: player.id(),
        ...
        adsResponse: '<VAST version="3.0"/>',
        // We will setup it when we need to show an ad
        ...
        locale: 'es',
        adLabel: 'Ad',
        vpaidMode: vpaidModeForced
      };
      if (playerStorage.options.vidAdOnFirstPreroll) {
        if (playerStorage.playerState.viewabilityState.isPrerollMandatory()) {
          //console.log("Trying preroll", playerStorage.options.adBreakOnlyPrerolls, playerStorage.options.viewability.firstVidAdMilliseconds);
          playerStorage.playerState.adManager.prepareToLaunchFirstPreroll(imaOptions);
          var done = function done(x) {
            (0, _log.cLog)(x);
            playerStorage.playerState.adManager.adManagerPrepared = true;
          };
          var millisecondsMaxWait = playerStorage.player.paused() ? 0 : 6000;
          new _trackAdRequestDone.TrackAdRequestDone(playerStorage.player, playerStorage.playerState, playerStorage.playerState.adManager.showRequestedAd, done.bind(null, "AdStarted"), done.bind(null, "AdError"), millisecondsMaxWait);
      ...
      player.one(['adserror', 'adend'], function () {
        setTimeout(function () {
          if (playerStorage.options.autoPlay && playerStorage.options.muted && playerStorage.player.paused()) {
            (0, _log.cLog)("Autoplay launched");
      ...
      var closeVidCoSetup = function closeVidCoSetup() {
        if (playerStorage.options.closeVidCo) {
          var closeVidCo = new _closeCSS.CloseCSS(videoEl.parentElement, playerStorage.rootDocument, function () {
            playerStorage.playerState.userClosedVidCo = true;
          });
          if (playerStorage.options.closeVidCoSize) closeVidCo.size = playerStorage.options.closeVidCoSize;
          closeVidCo.setup();
          closeVidCo.hide();
          var observerForCloseVidCo = function (playerState, closeVidCo) {
            if (playerState.adManager.tsLastAdLaunched && playerState.playingVidCo) {
              closeVidCo.show();
            }
          }.bind(this, playerStorage.playerState, closeVidCo);
          playerStorage.playerState.observe('startVidCo', observerForCloseVidCo);
          playerStorage.player.on(['adserror', 'aderror'], function () {
            observerForCloseVidCo();
            setTimeout(observerForCloseVidCo, 500);
            setTimeout(observerForCloseVidCo, 1100);
          });
        }
      };
      closeVidCoSetup();
      function firstAdDetection() {
        if (playerStorage.options.closeVidCoOnAdEnd) playerStorage.playerState.adManager.observe('endedVidAd', function () {
          var signal = function signal() {
            playerStorage.playerState.signalObservers('closedVidCo');
            /*DEPRECATED, ONLY USED BY Minijuegos*/
            playerStorage.playerState.signalObservers('closeVidCo');
            /*END DEPRECATED*/
          };

          if (playerStorage.options.closeVidCoDelayedMS > 0) setTimeout(signal, playerStorage.options.closeVidCoDelayedMS);else signal();
        });
        if (playerStorage.options.closeVidCoIfNoAd) playerStorage.playerState.adManager.observe('errorVidAd', function () {
          var signal = function signal() {
            playerStorage.playerState.signalObservers('closedVidCo');
            /*DEPRECATED, ONLY USED BY Minijuegos*/
            playerStorage.playerState.signalObservers('closeVidCo');
      ...
      firstAdDetection();
      playerStorage.player.one(['click', 'touchend'], function () {
        return playerStorage.player.ima.initializeAdDisplayContainer();
      });
      (0, _log.cLog)("IMA options:", imaOptions, "Player muted", player.muted(), "Player volume:", player.volume());
      var ava;
      if (playerStorage.options.ava && playerStorage.options.ava !== 'disabled') ava = (0, _setup.install)(videoEl, playerStorage.getElementToFloat(), playerStorage.rootDocument, playerStorage.getParentElementToFloat(), playerStorage.options.ava, player, playerStorage, playerStorage.options.avaSize, playerStorage.options.avaCloseSize, playerStorage.options.avaCloseDelayOnVidAd, playerStorage.options.avaOnlyOnVidAd, playerStorage.force_ava, playerStorage.options.observers);else playerStorage.pollIsAvaOn = function () {
        return false;
      };
      if (playerStorage.options.ava && playerStorage.options.ava !== 'disabled') {
        imaOptions.nonLinearWidth = ava.avaSize.getWidth();
        imaOptions.nonLinearHeight = ava.avaSize.getHeight();
        //console.log("AVA IMAOPTIONS", imaOptions, ava.avaSize)
      }

      if (playerStorage.options.vidAdNonLinearAdMaxWidth) imaOptions.nonLinearWidth = playerStorage.options.vidAdNonLinearAdMaxWidth;
      if (playerStorage.options.vidAdNonLinearAdMaxHeight) imaOptions.nonLinearHeight = playerStorage.options.vidAdNonLinearAdMaxHeight;
      if (typeof player.ima === 'function') player.ima(imaOptions);
      playerStorage.playerState.adManager.updateMuteAutoPlay();
      playerStorage.playerState.adManagerPluginInstalled = true;
      if (playerStorage.options.autoPlay) {
        playerStorage.player.one(['play'], function () {
          setTimeout(function () {
            if (!playerStorage.statusViewableVidCo.viewable) {
              (0, _log.cLog)("Not viewable yet. We pause until the user reaches that part", playerStorage.statusViewableVidCo);
              player.pause();
            }
          }, 3000);
        });
      }
      playerStorage.playerState.adManager.observe('endedVidAd', function () {
        setTimeout(function () {
          if (playerStorage.player && typeof playerStorage.player.paused === "function" && playerStorage.player.paused()) {
            (0, _log.cLog)("After Ad, player should be Playing. Do Play");
            playerStorage.player.play();
          }
        }, 800);
      });
    };
    //    if(0) resetVideoElement(playerStorage.player.id(),ready);
    //    else setTimeout(ready,2000);
    ready();
  };
  addIMA3Script(playerStorage.rootDocument, playerStorage.useShadowRoot, ima_loaded, ima3RequiresAnAccessibleScript, _typeof(playerStorage.options) === 'object' && playerStorage.options.dev);
};
var phase3Exec,
...
if (configID && !phase3Exec) ampPhase3(configID);
if (configID === 0 && !phase3Exec) {
  top.ampTV.ampPhase3 = ampPhase3;
  addIMA3Script(document, true, function () {
    var _this = this;
    window.player.ready(function (x) {
      exports['video.js'] = videojs;
      (0, _log.cLog)("Init ads", _this.ads);
      if (typeof _this.ads === 'function') _this.ads();
      require("videojs-ima");
      player.ima({});
    });
  }, ima3RequiresAnAccessibleScript, false);
}
...
function _toPropertyKey(arg) { var key = _toPrimitive(arg, "string"); return _typeof(key) === "symbol" ? key : String(key); }
function _toPrimitive(input, hint) { if (_typeof(input) !== "object" || input === null) return input; var prim = input[Symbol.toPrimitive]; if (prim !== undefined) { var res = prim.call(input, hint || "default"); if (_typeof(res) !== "object") return res; throw new TypeError("@@toPrimitive must return a primitive value."); } return (hint === "string" ? String : Number)(input); } // The playList has in the sources key, a description of the sources. Within it, an mp4 with the first 15 seconds.
function getGAM(playList) {
  var res = [];
...
function populatePlaylist(mobile, vidCoObj, cb) {
  var playList = [];
  vidCoObj.resolveMissing(mobile, /* resolveAllDone */function (values) {
    playList = vidCoObj.getPlaylistWithPlayableItems(values);
    if (!playList.length) (0, _log.cLog)("Missing creatives"); // continueVideoJSContentWhenAdDone will be false, and show_end() will proceed as configured
    if (cb) cb(vidCoObj, playList);
  });
  return playList;
}
var interval;
function monitorPlaylist(vidCoObj, options, player) {
  var minDepth = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 3;
  if (interval) clearInterval(interval);
  interval = setInterval(checkPlaylist.bind(this, vidCoObj, options, player, minDepth), 2000);
}
var allowChangeCurrent = true;
function checkPlaylist(vidCoObj, options, player) {
  var minDepth = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 3;
  if (!options.playListKWs || options.playListKWs.length <= 0) return;
  var depth = vidCoObj.getQueueDepth(player);
  //console.log("checkPlaylist",depth,minDepth);
  if (depth < minDepth) {
    vidCoObj.addNewItemBasedOnKeywords(options.vidCoAdUnit, options.size, options.playListKWs, function (x) {
      return vidCoObj.addItemToPlaylist(x, false) + vidCoObj.switchPlaylist(player, allowChangeCurrent);
    });
    allowChangeCurrent = false;
  }
}
